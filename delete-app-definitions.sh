...
appDefinitions=`curl -sS -XGET "$bigboatUrl/api/v2/apps/" -H "api-key:$apiKey"`
...
do
  ...
  appDefinition=`curl -sS -XGET "$bigboatUrl/api/v2/apps/$appName/$appVersion/files/bigboatCompose" -H "api-key:$apiKey"`
  ...
  then
    ...
    curl -sS -XDELETE "$bigboatUrl/api/v2/apps/$appName/$appVersion" -H "api-key:$apiKey"
  else
